...
def training(rank, conf, output_dir, args):
    print(f"Starting training on process {rank}")
    if args.restore:
    ...
    if args.distributed:
        ...
        device = f"cuda:{args.cuda_ids[0]}" if torch.cuda.is_available() else "cpu"
...
def parse_int_list(arg):
    try:
        ...
        raise argparse.ArgumentTypeError(f"Invalid list format: {arg}. Must be integers separated by commas.")
...
if __name__ == "__main__":
    ...
    if args.distributed or args.cuda_ids is not None:
        if args.cuda_ids is None:
          args.n_gpus = torch.cuda.device_count()
          args.cuda_ids = list(range(args.n_gpus))
        else:
          args.n_gpus = len(args.cuda_ids)
